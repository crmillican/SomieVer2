...
class AuthManager: ObservableObject {
    ...
    /// Fetch the business profile after authentication
    func fetchBusinessProfile() {
        guard let userId = currentUser?.id, let token = authToken else {
            return
        }
        
        var request = URLRequest(url: URL(string: "\(baseURL)/api/business/profile/\(userId)")!)
        request.httpMethod = "GET"
        request.setValue("Bearer \(token)", forHTTPHeaderField: "Authorization")
        
        URLSession.shared.dataTaskPublisher(for: request)
            .map { $0.data }
            .decode(type: BusinessProfile.self, decoder: JSONDecoder())
            .receive(on: DispatchQueue.main)
            .sink(receiveCompletion: { [weak self] completion in
                if case .failure(let error) = completion {
                    self?.error = "Failed to fetch business profile: \(error.localizedDescription)"
                }
    ...
    private func refreshUserProfile() {
        ...
        URLSession.shared.dataTaskPublisher(for: request)
            ...
            .sink(receiveCompletion: { [weak self] completion in
                ...
                // If it's a business user, fetch the business profile
                if user.isBusiness {
                    self?.fetchBusinessProfile()
                }
            })
            .store(in: &cancellables)
    }
